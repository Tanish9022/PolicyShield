...
// Chaos injection flag for Test 6
...
async function runLiveTests() {
  ...
  // Initialize DB baseline without closing connection
  await seed(false);
  ...
  const tests = [
    ...
    { name: 'Discount violation', input: 'Give me a 20% discount on the MacBook Pro', expected: 'APPROVE', checkAdapted: true },
  ...
  try {
    ...
    // Expected EXECUTION_UNKNOWN because of our injectTimeout wrapper
  }
  injectTimeout = false;
  ...
  // Branch B: Missing on Razorpay side
  ...
  const graph = await getPolicies(merchantId);
  if (graph) {
    graph.rules.push({
      policy_id: uuidv4() as any,
  ...
  await db.prepare('UPDATE inventory SET stock_level = ? WHERE merchant_id = ?').run(0, merchantId);
  try {
  ...
  await db.prepare('UPDATE products SET price = ? WHERE product_id = ?').run(200000, 'prod_macbook');
  try {
  ...
  await db.prepare('DELETE FROM webhook_events WHERE event_id = ?').run('evt_dup');
  ...
  console.log('Test 10: Policy version race');
  ...
  const currentVersion = await db.prepare('SELECT version FROM policy_versions WHERE merchant_id = ? ORDER BY id DESC LIMIT 1').get(merchantId) as any;
  if (currentVersion) {
    await db.prepare('UPDATE policy_versions SET version = ? WHERE merchant_id = ? AND version = ?').run(uuidv4(), merchantId, currentVersion.version);
  }
  try {
  ...
  // Test 11: Discount Precedence (Requested > Policy Cap)
  console.log('Test 11: Discount Precedence (Policy 5% < Requested 20%)');
  // Set policy to 5%
  const t11graph = await getPolicies(merchantId);
  const t11rule = t11graph?.rules.find((r: any) => r.rule_type === 'MAX_DISCOUNT');
  if (t11rule && t11graph) {
    t11rule.parameters.max_discount_percent = 5;
  ...
  const t11Intent: IntentRequest = {
    request_id: uuidv4() as any, intent_id: uuidv4() as any, merchant_id: merchantId as any,
    buyer_input: 'Give me a 20% discount on the AirPods Pro', received_at: new Date().toISOString()
  };
  ...
  // Test 12: Discount Precedence (Requested > Policy Cap 15%)
  console.log('Test 12: Discount Precedence (Policy 15% < Requested 20%)');
  // Restore policy to 15%
  if (t11rule && t11graph) {
    t11rule.parameters.max_discount_percent = 15;
    await db.prepare('UPDATE policy_versions SET rules_json = ? WHERE merchant_id = ? AND version = ?').run(JSON.stringify(t11graph.rules), merchantId, t11graph.version);
  }
  ...
  if (result12.gate_decision === 'APPROVE' && !razorpay_invoked12 && metadata12.final_discount === 15 && result12.action.state === 'VALIDATED') {
    console.log(`  Result: ✅ Approved correctly after adaptation. Razorpay NOT invoked during AI. Final discount 15%. State is VALIDATED.\n`);
  } else {
  ...
  console.log('Test 13: Concurrent Execution (TOCTOU)');
  ...
  const successCount = results13.filter(r => r && r.status === 'SUCCESS').length;
  ...
  if (hasFailures) {
    ...
    console.log('\n✅ ALL 13 ADVERSARIAL TESTS PASSED');
    process.exit(0);
  }
}

runLiveTests().catch(err => {
  console.error('Fatal live-tests error:', err);
  process.exit(1);
});
